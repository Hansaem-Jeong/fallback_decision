...
static __global__
    __launch_bounds__(512, 1) void DeepLearningNetwork_predict_kernel1(
        const unsigned char varargin_1[275598], cell_wrap_6 dataInputsSingle[1])
{
        ...
        const cell_wrap_6 dataInputsSingle[1], cell_wrap_9 inMiniBatchGroup[1])
{
  ...
  if ((static_cast<int>((static_cast<int>(p < 18)) &&
                                 ...
                                 float varargout_1[7])
{
  ...
  unsigned char(*gpu_varargin_1)[275598];
  cudaMalloc(&gpu_varargout_1, 28UL);
  cudaMalloc(&gpu_outMiniBatchGroup, 28UL);
  cudaMalloc(&gpu_inMiniBatchGroup, 1102392UL);
  cudaMalloc(&gpu_dataInputsSingle, 1102392UL);
  cudaMalloc(&gpu_varargin_1, 275598UL);
  cudaMemcpy(*gpu_varargin_1, varargin_1, 275598UL, cudaMemcpyHostToDevice);
  DeepLearningNetwork_predict_kernel1<<<dim3(539U, 1U, 1U),
                                        dim3(512U, 1U, 1U)>>>(
      *gpu_varargin_1, *gpu_dataInputsSingle);
  DeepLearningNetwork_predict_kernel2<<<dim3(539U, 1U, 1U),
                                        dim3(512U, 1U, 1U)>>>(
